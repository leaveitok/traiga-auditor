...
gcloud secrets add-iam-policy-binding %SECRET_NAME% --member=serviceAccount:%CR_SA% --role=roles/secretmanager.secretAccessor --project=%PROJECT_ID% --quiet

REM -- Step 3: Build Docker image -----------------------------------------------
echo [3/5] Building Docker image via Cloud Build...

gcloud artifacts repositories describe %IMAGE_REPO% --location=%REGION% --project=%PROJECT_ID% >nul 2>&1
if %errorlevel% neq 0 (
    echo   Creating Artifact Registry repo: %IMAGE_REPO%
    gcloud artifacts repositories create %IMAGE_REPO% --repository-format=docker --location=%REGION% --project=%PROJECT_ID% --quiet
)

gcloud builds submit "%BACKEND_DIR%" --tag=%IMAGE_URI% --project=%PROJECT_ID% --timeout=20m
if %errorlevel% neq 0 goto error
...
gcloud run deploy %SERVICE_NAME% --image=%IMAGE_URI% --region=%REGION% --platform=managed --no-allow-unauthenticated --env-vars-file="%ENV_FILE%" --set-secrets=/secrets/service_account.json=%SECRET_NAME%:latest --memory=2Gi --cpu=1 --min-instances=0 --max-instances=3 --timeout=300 --concurrency=80 --project=%PROJECT_ID% --quiet
if %errorlevel% neq 0 goto error

gcloud run services add-iam-policy-binding %SERVICE_NAME% --region=%REGION% --member=allUsers --role=roles/run.invoker --project=%PROJECT_ID% --quiet
