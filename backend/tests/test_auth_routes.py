...
Tests verify:
  - Admin users get role=platform_admin from /me
  - Non-admin users resolve to role=viewer from /me (multi-tenant taxonomy)
...
def test_get_me_admin(admin_client):
    """Admin email resolves to role=platform_admin with no city assignment."""
    r = admin_client.get("/api/auth/me")
    assert r.status_code == 200
    profile = r.json()
    assert profile["role"] == "platform_admin"
    assert profile["city"] is None


def test_get_me_city_user(city_client):
    """A non-admin user with no provisioned record resolves to viewer (fail-secure)."""
...
def test_upsert_user_admin_success(admin_client):
    """Admin can create a user (multi-tenant payload: role + cities list)."""
    payload = {"email": "new@city.gov", "role": "viewer", "cities": ["New City"]}
    r = admin_client.post("/api/auth/users", json=payload)
    assert r.status_code == 200


def test_upsert_user_city_forbidden(city_client):
    """City-scoped (viewer) user cannot create users."""
    payload = {"email": "new@city.gov", "role": "viewer", "cities": ["New City"]}
...
def test_upsert_user_persists_to_repo(admin_client, empty_repo):
    """Upserted user must be written to the repository with its granted cities."""
    payload = {"email": "officer@lewisville.gov", "role": "viewer",
               "cities": ["City of Lewisville"]}
    r = admin_client.post("/api/auth/users", json=payload)
    assert r.status_code == 200
    user = empty_repo.get_user("officer@lewisville.gov")
    assert user is not None
    assert user["cities"] == ["City of Lewisville"]
    assert user["city"] == "City of Lewisville"   # legacy single-value mirror
