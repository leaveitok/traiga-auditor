...
| `SB1964_FRAMEWORK_DESIGN.md` | ✅ SHIPPED | Texas SB 1964 government AI code-of-ethics **lens** — 14-control crosswalk (sb1964_ref + overlap, 9 strong / 5 partial), framework registry entry (TX, mandatory, on by default), `FRAMEWORK_SB1964_ENABLED` setting, framework-parameterized Alignment Statement docx. Governance-as-code: NO engine change (evaluate_profile untouched). Dimension-level mapping; revisit when DIR rules publish. |
| `EVIDENCE_BUNDLES_DESIGN.md` | 🟡 PARTIAL | Audience-tailored evidence bundles. **Phases 1–2 shipped**: bundle engine, Council Brief + AG/Auditor Package presets, PDF+DOCX, zip package, tamper-evident content hash, provenance-first, candidate-not-verdict, Reports section with live preview, **Evidence Room** (immutable snapshots frozen as model JSON, re-render on demand, live-fingerprint stale detection, tombstone delete, snapshot Protocol methods in Mock/Sheets/Firestore). Phase 3 (fast-follow audiences) and Phase 4 (product-docs refresh + skill) **not built**. Prod GCS byte-storage remains a future ArtifactStore upgrade. |
